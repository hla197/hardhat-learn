...
contract NftAuctionV2 is NftAuction {
    ...
    function setPlatformFee(uint256 _platformFeeBps, address _platformFeeRecipient) public onlyOwner {
        platformFeeBps = _platformFeeBps;
        platformFeeRecipient = _platformFeeRecipient;
    }

    /// @custom:oz-upgrades-unsafe-allow-reachable
    function initializeV2(uint256 _platformFeeBps, address _platformFeeRecipient) public reinitializer(2) {
        platformFeeBps = _platformFeeBps;
        platformFeeRecipient = _platformFeeRecipient;
    }

    // 重写 endAuction 方法，添加平台费用逻辑
    function endAuction(uint256 auctionId) public override auctionActive(auctionId) {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            if (auction.highestBidderToken == address(0)) {
                ...
                IERC20(auction.highestBidderToken).transfer(auction.seller, sellerAmount);
                // 转移平台费用给平台
                IERC20(auction.highestBidderToken).transfer(platformFeeRecipient, platformFee);
            }
