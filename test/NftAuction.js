...
async function NftAuction() {
  ...
  async function initConfig() { 
      ...
      const USDCAggregator = await Mock.deploy(
        DECIMALS,
        0.98 * 10 ** DECIMALS
      );
      ...
      const ETHAggregator = await Mock2.deploy(
        DECIMALS,
        2980 * 10 ** DECIMALS
      );
  ...
  async function initData() {
      ...
      await nft.mint(
        addr1.address,
        "ipfs://bafkreig6zuolgmcskysrajqblmp3u6rp6vqhfgnscivx75ysizu7xp35re"
      );
      await nft.mint(
        addr2.address,
        "ipfs://bafkreig6zuolgmcskysrajqblmp3u6rp6vqhfgnscivx75ysizu7xp35re"
      ...
      await erc20
        .connect(owner)
        .transfer(addr1.address, ethers.parseUnits("200", 6));
      await erc20
        .connect(addr1)
        .approve(proxyAddress, ethers.parseUnits("200", 6));

      await erc20
        .connect(owner)
        .transfer(addr2.address, ethers.parseUnits("200", 6));
      await erc20
        .connect(addr2)
        .approve(proxyAddress, ethers.parseUnits("200", 6));
      ...
      await erc20
        .connect(owner)
        .transfer(addr3.address, ethers.parseUnits("200", 6));
      await erc20
        .connect(addr3)
        .approve(proxyAddress, ethers.parseUnits("200", 6));
  }
  ...
  describe("NftAuction Contract", function () {
    ...
    it("setPriceFeed", async function () {
      ...
      console.log(
        "ETH:",
        await proxy.getChainlinkDataFeedLatestAnswer(ethers.ZeroAddress)
      );
      console.log(
        "USDC:",
        await proxy.getChainlinkDataFeedLatestAnswer(UsdcAddress)
      );
    });
    ...
    it("创建拍卖1 USDC胜拍 Addr2 -200usdc +1nft, addr1 +200usdc -1nft", async function () {
      ...
      expect(
        await nft.connect(addr1).isApprovedForAll(addr1.address, proxyAddress)
      ).to.equal(true);
    ...
    it("创建拍卖2 无人竞拍的", async function () {
      ...
      expect(
        await nft.connect(addr2).isApprovedForAll(addr2.address, proxyAddress)
      ).to.equal(true);
    ...
    it("参与拍卖", async function () {
      ...
      await proxy
        .connect(addr3)
        .placeBid(1, UsdcAddress, ethers.parseUnits("100", 6));
      auction = await getAuctionDetails(1);
      ...
      await proxy
        .connect(addr2)
        .placeBid(1, UsdcAddress, ethers.parseUnits("200", 6));
      auction = await getAuctionDetails(1);
